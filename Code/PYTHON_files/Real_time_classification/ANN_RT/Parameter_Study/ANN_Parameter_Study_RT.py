...
for participant_no in range(1, 8):
    # Construct the folder path for the current participant
    folder_path = f'{base_folder_path}\\{participant_no}\\{folder_name}'
...
final_model = train_model(x_trainval, y_trainval, best_param.item(), log_dir='./logs/ANN_individual_realtime_model', final=True)
final_model.save('./Model/ANN_individual_realtime.h5') 
#load saved model and calcuate accuracy with test datasets
saved_model = tf.keras.models.load_model('./Model/ANN_individual_realtime.h5')
predictions = saved_model.predict(x_test)
predictions_df['Predictions'] = np.argmax(predictions, axis=-1)
print_confusion_matrix(y_test, predictions_df['Predictions'], 'Confusion Matrix for ANN real time classification')
