...
def features_estimation(signal, channel_name, fs, frame, step, plot=False):
    ...
    total_feature_matrix = pd.DataFrame(np.row_stack((time_matrix, frequency_matrix, time_frequency_matrix)),
...
def time_features_estimation(signal, frame, step):
    ...
    time_features_matrix = np.row_stack((variance, rms, iemg, mav, log_detector, wl, aac, dasdv, zc, wamp, myop))#zc,
...
# # -*- coding: utf-8 -*-
# """
# Created on Tue May 30 14:10:04 2023
...
#     features_names = ['VAR', 'RMS', 'IEMG', 'MAV', 'LOG', 'WL', 'ACC', 'DASDV', 'ZC', 'WAMP', 'MYOP'] #'Label'
#     #labels = label_estimation(signal.iloc[:, 1], label_name, frame, step)
#     time_matrix = time_features_estimation(signal, frame, step)
#     #total_feature_matrix = pd.DataFrame(np.column_stack(labels, time_matrix)),

#     print('EMG features were extracted successfully')

#     # if plot:
#     #     plot_features(signal, fs, total_feature_matrix, step)

#     return time_matrix, features_names

# def label_estimation(label, label_name, frame, step):
#     labels = []
#     for i in range(frame, label.size, step):
#         x = label[i - frame:i]
        
#         label_mean = np.mean(x)
#         if label_mean > 0.5:
#             label_mean = label_name
#         else: 
#             label_mean = 0
...
def time_domain_statistics(vector, frame, step):
    features = []
    for i in range(frame, vector.size, step):
        x = vector[i - frame:i]
        features.append(MAV(x))
        features.append(zero_crossing(x))
        features.append(slop_sign_change(x))
